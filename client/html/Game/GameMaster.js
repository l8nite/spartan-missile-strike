...
GameMaster.prototype._doFireOnService = function (gameid, loc, orientation, power) {
	...
	return $.ajax({
		url: this.Imports.serviceurl + "/games/" + encodeURIComponent(gameid) + "/fire-missile",
		type: "PUT",
...
GameMaster.prototype._getGamesFromService = function (fromWhen) {
	...
	return $.ajax({
		url: this.Imports.serviceurl + "/users/" + encodeURIComponent(this.userid) + "/games",
		headers: headers
...
GameMaster.prototype._getNameFromService = function (userid) {
	var d = new $.Deferred();
	$.ajax({
		url: this.Imports.serviceurl + "/users/" + encodeURIComponent(userid),
		headers: {
			"MissileAppSessionId": this._sessionid
		},
		dataType: "json"
